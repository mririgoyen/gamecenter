...
(function (Promise) {
  ...
  function IALoader(canvas, game, callbacks, scale) {
        ...
        () => {
          ...
          .then((data) => {
            ...
            if (module && module.indexOf('dosbox') === 0) {
              ...
              var fliplists = [];
              vice_fliplist.forEach((fliplist_meta) => {
                if (!fliplist_meta) {
                  fliplists.push(null);
                } else {
    ...
    function get_mame_files(cfgr, metadata, modulecfg, filelist) {
      ...
      var meta = dict_from_xml(metadata),
        peripherals = {},
        game_files_counter = {};
      files_with_ext_from_filelist(filelist, meta.emulator_ext).forEach((file, i) => {
      ...
      meta_props_matching(meta, /^mame_peripheral_([a-zA-Z0-9]+)$/).forEach((result) => {
      ...
      Object.keys(peripherals).forEach((periph) => {
  ...
  function Emulator(canvas, callbacks, loadFiles) {
        ...
        return new Promise((resolve, reject) => {
          ...
          if (BrowserFS.FileSystem.IndexedDB.isAvailable()) {
            var AsyncMirrorFS = BrowserFS.FileSystem.AsyncMirror,
              {IndexedDB} = BrowserFS.FileSystem;
            ...
            game_data.fs.initialize((e) => {
              if (e) {
                ...
                if ('emulatorWASM' in game_data && game_data.emulatorWASM && 'WebAssembly' in window) {
                  promises.push(fetch({ title: 'WASM Binary', url: game_data.emulatorWASM }).then((data) => {
    ...
    var resizeCanvas = function (canvas, scale, resolution) {
      if (scale && resolution) {
  ...
  var flag_r = { isReadable: function() {
    return true;
  },
  ...
  var flag_w = { isReadable: function() {
    return false;
  },
  isWriteable: function() {
    return true;
  ...
  function files_from_filelist(xml) {
    return list_from_xml(xml).filter((node) => {
  ...
  function files_with_ext_from_filelist(xml, ext) {
    ...
    ext = ext.toLowerCase();
    return files_from_filelist(xml).filter((file) => {
